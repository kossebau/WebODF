...
/**
 * @param {!{sessionId: string, username: string, sessionListElementId: string, editorContainerElementId: string}} args
 */
function createEditor(args) {
    ...
    function joinSession(sessionId) {
        ...
        server.joinSession(args.username, sessionId, function(memberId) {
            ...
            if (!editor) {
                window.wodo_plugins = [{
                    ...
                    config: {
                        ...
                        // annotations: true,
                        undo: false, // no undo manager yet
                        utterance: args.utterance,
                        utteranceLoud: args.utteranceLoud,
                        seed: undefined,
                        autoDrive: false,
                        debug: true,
                        statusContainer: document.getElementById(args.editorContainerElementId).parentNode,
                        actionTime: 50, // Sleep time between actions in ms
                        lengthCheck: 10
                    }
                }];
    ...
    window.jsGlobalInstance.registerUserData(args.username, args.userData);

    // start the editor with network
    runtime.log("starting collaborative editor for [jsglobal].");
    require(["webodf/editor/backend/jsglobal/ServerFactory"], function (ServerFactory) {
...
window.JsGlobalServer = (function() {
    ...
    function JsGlobalSession(title) {
         ...
         * @return {undefined}
         */
        this.addMember = function (memberId, userData) {
            var addMember = {
                optype: "AddMember",
                memberid: memberId,
                timestamp: Date.now(),
    ...
    function JsGlobalServer() {
        ...
        this.login = function (login, successCb) {
            ...
            processCallback(successCb, {
                full_name: userData[login].fullName,
                securityToken: login
            });
        };
         ...
         * @return {undefined}
         */
        this.joinSession = function (login, sessionId, successCb) {
            ...
            session.addMember(memberId, userData[login]);
         ...
         * @return {undefined}
         */
        this.registerUserData = function (login, data) {
            userData[login] = data;
        };
    }
